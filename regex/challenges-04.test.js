...
const isCapitalized = string => {
  // Solution code here...
  return string.match(/\b(\w*[A-Z]\w*)\b/g);
};
...
const citiesAtoJ = cities => {
  ...
  cities.forEach(element => {
    if (element.match(/\b(\w*[A-J]\w*)/)) {
      filter.push(element.match(/\b(\w*[A-J]\w*)/)[0]);
    }
  });
  return filter;
};
...
// CHALLENGE 7
//
// Write a function named findShells that takes in the string below and uses a regular
// expression pattern to find all instances of the following words: "sells", "shells", "seashells".
