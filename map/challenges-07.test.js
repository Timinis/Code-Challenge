...
Write a function named forLoopTwoToThe that, given an array of integers as input, iterates over the array and returns a new array. The returned array should containing the result of raising 2 to the power of the original input element.
...
const forEachTwoToThe = input => {
  // Solution code here...
  let newArr = [];
  input.forEach(element => newArr.push(Math.pow(2, element)));
  return newArr;
};
...
const mapTwoToThe = input => {
  // Solution code here...
  return input.map(element => Math.pow(2, element));
};
...
const charCode = input => {
  // Solution code here...
  return input.map(element => element.charCodeAt(0));
};
