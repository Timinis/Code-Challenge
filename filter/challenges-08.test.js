...
const oddValues = input => {
  // Solution code here...
  return input.filter(element => element % 2 === 1);
};
...
const filterStringsWithVowels = input => {
  // Solution code here...
  return input.filter(element => element.match(/[a e i o u]/gi));
};
...
const notInFirstArray = (forbiddenValues, input) => {
  ...
  for (let i = 0; i < forbiddenValues.length; i++) {
    for (let j = 0; j < input.length; j++) {
      if (forbiddenValues[i] === input[j]) {
        input[j] = false;
      }
    }
  }
  return input.filter(element => element !== false);
};
...
const getBaseStatGreaterThan = (input, minBaseStat) => {
  // Solution code here...
  return input.filter(element => element.baseStat > minBaseStat);
};
...
const getStatName = (input, minBaseStat) => {
  // Solution code here...
  return input
    .filter(element => element.baseStat > minBaseStat)
