...
const houseSize = arr => {
  let arrObj = Object.values(arr);
  const sizeConst = (house, members) => ({
    house,
    members
  });
  const householdCounter = obj => {
    let members = 0;
    members = obj.children.length + members;
    if (obj.spouse) {
      members++;
    }
    if (obj.name) {
  ...
  const sizes = [];
  arrObj.forEach(element => {
    sizes.push(sizeConst(element.house, householdCounter(element)));
  });
...
const houseSurvivors = arr => {
  ...
  const householdCounter = obj => {
    let members = 0;
    members = obj.children.length + members;
    deceasedSpouses.forEach(element => {
      if (element === obj.spouse) {
        obj.spouse = null;
      }
    });
    if (obj.spouse) {
  ...
  arrObj.forEach(element => {
    survivors.push(sizeConst(element.house, householdCounter(element)));
  });
